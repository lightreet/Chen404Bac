...
本文档描述当前已落地的 Lyra AI 后台配置能力。最后同步时间：2026-08-14。
...
- 音乐曲目信息补全复用全局 LLM 配置，其业务开关由“站点配置 > 功能开关”管理；文章 AI 助手和相关文章推荐也采用相同边界。
...
后台“AI 助手”配置与“功能开关”是两个相邻但不同的配置域：前者维护 provider、模型、Lyra 人设、聊天检索与工具参数；后者通过 `FeatureToggleService` 控制 AI 文章辅助、音乐补全和相关文章推荐是否开放。`webSearchEnabled` 仍只是 AI 配置中的预留能力，不代表已接入真实联网搜索。

...
- `/memory-map` 路由本身是公开入口，但真实内容和后台接口仍要求“管理员或知友”；普通登录用户与游客只会看到访问封面。
- 前端已经收敛为“高德真实底图优先，城市/省级 GeoJSON + 基础 SVG 回退”的展示态，并保留独立详情页和管理员编辑工作台。
...
| 路径 | 角色 | 权限 |
| --- | --- | --- |
| `/memory-map` | 主展示页 | 公开入口，内容仅管理员或知友可见 |
...
1. 左栏：旅行索引侧栏，负责地点切换、缩略图预览、管理员编辑/删除入口
2. 中栏：地图主视觉、当前位置、路线、地图操作与管理员新增入口
3. 右栏：当前地点的册页式详情、封面图、旅行碎片、引用语与“查看游记”主 CTA
...
- 返回地图入口
- 照片 hero、地点/日期上下文和整体感想
- 旅途片段列表与片段目录
- 位置概览、迷你地图、旅行相册
- 上一篇 / 下一篇导航

它适合承接“从地图当前地点进入，更专注地看某一段旅程”的场景。
...
后端最终权限判断由 `AccessService` 完成；前端 `/memory-map` 通过访问封面表达权限状态，详情/编辑等路由再叠加守卫与接口限制。
...
### 6.17 主展示页三栏 atlas 收敛记录

当前 `MemoryMap.vue` 已按“左侧旅行索引栏 + 中间地图主画布 + 右侧地点详情栏”的同屏 atlas 工作区收敛。本章节保留结构 rationale、影响范围与后续拆分建议，供后续迭代继续参考。

...
最近一次按代码校准时间：2026-08-14。

## 1. 当前结论

- 旅行纪念地图已经不是纯规划功能，而是当前前后端均已落地的正式模块。
- 当前主入口是前端 `/memory-map`，以“左侧旅行索引 + 中间地图主画布 + 右侧当前地点详情”的 atlas 工作区呈现。
- `/memory-map` 和后端读取接口允许游客访问公开旅行地点；知友和管理员可在公开内容基础上读取 `friend` 内容，不再使用整页访问封面阻断公开内容。
- 前端已经收敛为“高德真实底图优先，城市/省级 GeoJSON + 基础 SVG 回退”的展示态；地点详情在右侧面板展示，旧详情路径只负责重定向到 `/memory-map?focus={id}`。
- 启用中的知友与管理员可进入创建工作台；内容所有者或管理员可编辑、删除，后台管理接口继续作为兼容和全站治理入口。
- 后端已接入地点、片段、照片的结构化保存，保留旧版扁平 `entries` 兼容输入，完成图片 EXIF 解析、文件转永久与统一引用同步。
...
1. 展示公开与知友可见的旅行地点和照片故事
2. 作为知友创作者与管理员可维护的旅行内容工作台
...
| 路径 | 角色 | 权限 |
| --- | --- | --- |
| `/memory-map` | 主展示页 | 公开内容对所有访客可见，知友/管理员扩展读取知友内容 |
| `/memory-map/detail/:id` | 旧链接兼容 | 重定向到 `/memory-map?focus={id}`，不再有独立详情组件 |
| `/memory-map/create` | 创建工作台 | 具备 `travel:create` capability 的知友或管理员 |
| `/memory-map/edit/:id` | 编辑工作台 | 内容所有者或管理员，后端最终鉴权 |
...
`MemoryMap.vue` 当前不是单一地图板，而是三栏 atlas 工作区：

1. 左栏：旅行索引侧栏，负责地点切换、缩略图预览、所有者/管理员编辑删除入口
2. 中栏：地图主视觉、当前位置、路线、地图操作与有创作能力用户的新增入口
3. 右栏：当前地点的册页式详情、封面图、旅行碎片、引用语与所有者管理入口

当前交互特征：

- 所有访客先加载当前身份可见的地点；没有公开内容时展示普通空状态/访问提示
- 知友和管理员可额外看到 `friend` 内容
- 具备创作能力的用户可直接跳转新建，所有者或管理员可编辑/删除
- URL 中的 `focus` 用于定位并展开指定地点，承接旧详情链接和保存后的返回定位

## 3.3 地点详情面板

独立 `TravelMemoryDetail.vue` 已移除。当前地点详情由 `MemoryMap.vue` 右侧面板承载，集中展示封面、地点/日期、旅行片段、照片、感想与地图上下文。路由 `/memory-map/detail/:id` 仅保留旧链接兼容，并重定向到带 `focus` 的主页面。
...
- 展示态优先加载高德真实底图，缺少 Key 或脚本失败时回退到城市/省级 GeoJSON 与基础 SVG
- 编辑态使用高德地图脚本接入、定位和逆地理编码
- 支持边界点击、城市匹配、点位高亮、标签避让、路线绘制与多地点同边界选择
- 当前 `TravelMemoryMap.vue` 仍是单文件门面，后续宜继续拆分
...
- 一个地点优先包含多个旅途片段（stop），每个片段下再挂照片条目
- 为兼容旧数据和旧接口输入，后端仍保留扁平 `entries` 的 fallback 读取与写入归一化
- 照片条目可保存备注、感想、拍摄时间、展示顺序、是否封面、坐标来源
- 当前前端主流程仍主要维护地点级展示坐标；片段级坐标字段已在契约中保留，后端会在地点坐标缺失时按“片段坐标 -> EXIF 坐标”继续回填

## 3.9 权限模型

可见性：

- `public`：游客、登录用户、知友、所有者和管理员都可查看。
- `friend`：知友、所有者和管理员可查看。
- 管理：创建者可管理自己的地点，管理员可管理全站地点；写入入口还受 `travel:create` capability 与运行时开关约束。

后端最终权限判断由 `AccessService` 和业务服务完成；前端根据 capability 控制入口、根据接口返回展示可见内容，不能替代服务端对象级鉴权。
...
- 地点、片段、照片已经收敛为稳定的 location / stop / entry 模型
- 旧版扁平 `entries` 输入仍保留为兼容层，由服务端统一归一化为 stop 结构
- 保存时统一同步文件引用
- 图片上传链路与 EXIF 解析已贯通
...
当前 `TravelMemoryMap.vue` 已经完成“高德地图承载真实坐标，自定义标签承载 Chen404 视觉表达”的第一阶段收敛。本章节保留两层信息：一是当前真实实现的技术结论，二是后续继续拆分单文件门面时可参考的模块化建议。
...
| 场景 | 当前实现 | 主要用途 |
| --- | --- | --- |
| 展示态默认路径 | 高德 JS API 2.0 + 自定义 marker + 路线绘制 | `/memory-map` 主展示页 |
| 展示态 fallback | 城市级 GeoJSON + D3 `geoMercator` + SVG marker | 高德 Key 缺失、脚本失败或网络异常时 |
...
当前代码仍集中在 `TravelMemoryMap.vue` 单文件中。下面的模块图和拆分边界用于后续维护，不代表这些文件现在都已存在。
...
### 6.17 主展示页三栏 atlas 收敛记录（历史实施 rationale）

当前 `MemoryMap.vue` 已按“左侧旅行索引栏 + 中间地图主画布 + 右侧地点详情栏”的同屏 atlas 工作区收敛。本章节保留当时的结构 rationale 与实施步骤；其中“管理员入口”“独立详情页”等措辞是改造阶段语境，当前权限和路由必须以第 3 节为准。
...
| 目标 | 维持有能力创作者的新建入口、所有者/管理员编辑删除入口可达，但不破坏浏览主线 |
| 非目标 | 本阶段不重做旅行地点后端数据模型 |
| 非目标 | 本阶段不新增后端接口、分页 API 或独立搜索接口 |
| 非目标 | 当时阶段不重写独立详情页；该组件后续已移除，旧路由现重定向到主页面 `focus` 状态 |
...
- 保留具备 `travel:create` capability 用户的新增地点入口
- 地图应继续是整页最大的视觉区域

**右侧当前地点详情栏**

- 保留标题、地点、日期、封面图、旅行碎片与整体感想
...
4. 右侧详情栏切换为该地点详情
5. 用户在右侧详情区继续切换和阅读片段，不再跳转独立详情页

补充规则：

- 左侧条目点击不直接跳转独立详情页。
- 左侧条目不再单独放“查看游记”按钮，避免与右侧主阅读区竞争。
- 所有者/管理员操作入口放在左侧条目或当前详情区的轻量菜单中；具体状态变量以现有实现为准。
...
5. 同一屏只保留与当前地点相关的明确主操作，避免索引条目与详情区重复 CTA。
...
8. 所有者/管理员操作入口保持可达，确保编辑/删除功能不回退。
...
- `MemoryMap.vue`、`TravelMemoryCreate.vue`、`TravelMemoryMap.vue` 体量仍偏大，后续宜继续拆成稳定的子组件或 composable
- 旅行纪念地图的自动化测试仍偏薄，建议补权限态、加载失败、focus 返回、高德 fallback 等关键链路

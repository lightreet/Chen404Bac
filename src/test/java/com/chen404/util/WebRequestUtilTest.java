...
class WebRequestUtilTest {
    ...
    @Test
    void shouldIgnoreSpoofedForwardedHeaders() {
        MockHttpServletRequest request = new MockHttpServletRequest();
        request.addHeader("X-Forwarded-For", " 203.0.113.10, 10.0.0.2 ");
        request.setRemoteAddr("10.0.0.1");

        assertEquals("10.0.0.1", WebRequestUtil.getClientIp(request));
    }

    @Test
    void shouldIgnoreOtherClientControlledIpHeaders() {
        MockHttpServletRequest request = new MockHttpServletRequest();
    ...
    @Test
    void shouldNormalizeRemoteIpv6Loopback() {
        MockHttpServletRequest request = new MockHttpServletRequest();
